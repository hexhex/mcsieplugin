...
void
   InputConverter::convertParseTreeToDLVProgramEx(node_t& node, std::ostream& o) {
     if (!(node.value.id() == MCSdescriptionGrammar::Root)) {
       throw PluginError("MCS Equilibrium Plugin: Inputfile syntax error!");
     }
     bridgerules.clear();
   ...
   InputConverter::convert(std::istream& i, std::ostream& o) {
	...
	convertParseTreeToDLVProgramEx(*info.trees.begin(), ss);
	//guess a subset of the candidate explanation
	// for which holds that E1 C_ r1 C_ brM and r2 C_ brM \ E2
	ss << "r1(R) :- e1(R)." << std::endl;
	ss << "r1(R) v nr1(R) :- ne1(R)." << std::endl;
	ss << "r2(R) v nr2(R) :- ne2(R)." << std::endl;
	// ensure saturation
	ss << ":- not spoil." << std::endl;
